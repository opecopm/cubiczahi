...
class Create extends Component
{
    ...
    public function mount()
    {
        ...
        if ($salesOrderId = request()->query('sales_order')) {
            $this->loadFromSalesOrder($salesOrderId);
        }
    }

    public function loadFromSalesOrder($id)
    {
        $salesOrder = SalesOrder::with(['customer', 'items'])->find($id);
        if ($salesOrder) {
            $this->sales_order_id = $salesOrder->id;
            $this->sales_order_reference = $salesOrder->reference;

            if ($salesOrder->customer) {
                $this->customer_id = $salesOrder->customer->id;
                $this->customer_reference = $salesOrder->customer->reference;
            }

            foreach ($salesOrder->items as $orderItem) {
                $item = Item::find($orderItem->item_id);
                $tax = Tax::find($orderItem->tax_id ?? null);
                
                $this->items[] = [
                    'item_id' => $orderItem->item_id,
                    'name' => $orderItem->name,
                    'description' => $orderItem->description ?? ($item ? $item->getTranslation('name', 'ar') : null),
                    'quantity' => $orderItem->quantity,
                    'unit' => $orderItem->unit,
                    'price' => $orderItem->price,
                    'total_price' => $orderItem->total_price ?? ($orderItem->price * $orderItem->quantity),
                    'discount_type' => $orderItem->discount_type ?? 'fixed',
                    ...
                    'tax_rate' => $tax->rate ?? 0,
                    'tax_amount' => $orderItem->tax ?? 0,
                    'total' => $orderItem->total ?? 0,
                    'is_rental' => $orderItem->is_rental ?? 0,
                    'rental_start_at' => $orderItem->rental_start_at ?? null,
                    'rental_end_at' => $orderItem->rental_end_at ?? null,
                ];
            }
    ...
    public function selectSalesOrder($reference)
    {
        ...
        if ($salesOrder) {
            $this->items = []; // Clear current items before loading
            $this->loadFromSalesOrder($salesOrder->id);
        } else {
    ...
    public function store()
    {
        ...
        try {
            $salesInvoice = DB::transaction(function () use ($validated) {
            ...
            report($e);
            session()->flash('error', 'Failed to create Sales Invoice. Nothing was saved.');
    ...
    public function render()
    {
        ...
        $nextReference = ReferenceSchema::getNextReference('sales_invoice') ?? '';

        return view('selling::livewire.sales-invoices.create', compact(
            'units',
            'customers',
            'all_items',
            'taxes',
