@foreach($departments as $index => $department)
    <tr>
        <td class="text-secondary">{{ $prefix . ($index + 1) }}</td>
        <td>
            {!! str_repeat('<span class="text-secondary me-1">—</span>', $level) !!}
            <span class="fw-bold">{{ $department->name }}</span>
        </td>
        <td class="text-secondary">{{ $department->type }}</td>
        <td>
            <span class="badge {{ $department->status === 'active' ? 'bg-success-lt' : 'bg-secondary-lt' }}">
                {{ ucfirst($department->status) }}
            </span>
        </td>
        <td class="text-secondary">{{ $department->parent->name ?? '' }}</td>
        <td class="text-secondary">{{ trim(($department->hodUser->first_name ?? '') . ' ' . ($department->hodUser->last_name ?? '')) ?: '—' }}</td>
        <td>
            <div class="btn-group">
                <button wire:click="edit({{ $department->id }})" class="btn btn-sm btn-ghost-secondary">
