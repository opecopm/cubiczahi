    ...
    private function importPrice(Collection $rows): void
    {
        foreach ($rows as $i => $row) {
            ...
            if ($existing && $existing->type !== 'product') {
                $this->errors[] = "Row {$rowNum}: item with reference '{$reference}' is type '{$existing->type}', expected 'product'.";
                $this->skipped++;
    ...
    private function importQuantity(Collection $rows): void
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            if ($item->type !== 'product') {
                $this->errors[] = "Row {$rowNum}: item with reference '{$reference}' is type '{$item->type}', expected 'product'.";
                $this->skipped++;
