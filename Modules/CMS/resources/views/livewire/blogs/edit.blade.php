            ...
            <form wire:submit.prevent="update" enctype="multipart/form-data">
                ...
                <div class="row">
                            ...
                            <div class="card-body">
                                    ...
                                    @foreach($languages as $code => $label)
                                        <li class="nav-item" role="presentation">
                                            <button class="nav-link @if($loop->first) active @endif" id="tab-{{ $code }}"
                                                data-bs-toggle="tab" data-bs-target="#content-{{ $code }}" type="button"
                                                role="tab">
                                ...
                                <div class="tab-content">
                                    @foreach($languages as $code => $label)
                                        <div class="tab-pane fade @if($loop->first) show active @endif"
                                            id="content-{{ $code }}" role="tabpanel">

                                            <div class="mb-3">
                                                <label class="form-label fw-semibold">Title</label>
                                                <input type="text" wire:model.defer="title.{{ $code }}"
                                                    class="form-control fs-4 @error('title.' . $defaultLocale) is-invalid @enderror"
                                                    placeholder="Enter title" required>
                                                @error('title.' . $defaultLocale)
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" wire:model.defer="slug.{{ $code }}"
                                                    class="form-control @error('slug.' . $defaultLocale) is-invalid @enderror"
                                                    placeholder="Enter slug">
                                                @error('slug.' . $defaultLocale)
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="mb-3" wire:ignore>
                                                ...
                                                <div id="editor-{{ $code }}" class="bg-white" style="height: 500px;">
                                                    {!! $content[$code] ?? '' !!}
                                                </div>
                                                @error('content.' . $defaultLocale)
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea wire:model.defer="excerpt.{{ $code }}"
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" wire:model.defer="allow_comments" id="allow_comments">
                                        <label class="form-check-label" for="allow_comments">Allow comments</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" wire:model.defer="allow_pings" id="allow_pings">
                                        <label class="form-check-label" for="allow_pings">Allow trackbacks and pingbacks</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-lg-4">
                            ...
                            <div class="card-body">
                                @foreach($categories as $category)
                                    <div class="form-check ms-0">
                                        <input type="checkbox" value="{{ $category->id }}" wire:model.defer="selectedCategoryIds"
                                            class="form-check-input" id="cat{{ $category->id }}">
                                        <label class="form-check-label"
                                            for="cat{{ $category->id }}">{{ $category->getTranslation('name', app()->getLocale()) ?? $category->getTranslation('name', $defaultLocale) }}</label>
                                    </div>

                                    @if($category->children && $category->children->count())
                                        @foreach($category->children as $sub)
                                            <div class="form-check ms-4">
                                                <input type="checkbox" value="{{ $sub->id }}" wire:model.defer="selectedCategoryIds"
                        ...
                        <!-- Featured Image -->
                        <div class="card mb-3" x-data="{ imagePreview: @if($blog->featured_image) '{{ asset('storage/'.$blog->featured_image) }}' @else null @endif }">
                            <div class="card-header">
                        ...
                        @if($isChild)
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('parentId') is-invalid @enderror" wire:model.defer="parentId">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}">{{ $category->getTranslation('name', app()->getLocale()) ?? $category->getTranslation('name', $defaultLocale) }}</option>
                                    @endforeach
                                </select>
